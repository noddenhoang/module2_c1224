package smartphone_managerment;

import java.math.BigDecimal;
import java.util.*;

public class GUI extends Smartphone {
    static Scanner sc = new Scanner(System.in);
    private int choice;

    public void guiMain() {
        do {
            System.out.println("-- CHƯƠNG TRÌNH QUẢN LÝ ĐIỆN THOẠI --");
            System.out.println("1. Xem danh sách điện thoại.");
            System.out.println("2. Thêm điện thoại.");
            System.out.println("3. Sửa thông tin điện thoại.");
            System.out.println("4. Xóa điện thoại.");
            System.out.println("5. Sắp xếp theo giá.");
            System.out.println("6. Tìm kiếm điện thoại.");
            System.out.println("7. Tính tổng tiền.");
            System.out.println("8. Giảm giá cho điện thoại cũ.");
            System.out.println("9. Thoát.");
            System.out.print("Chọn chức năng: ");
            choice = Integer.parseInt(sc.next());
            String choiceStr;
            switch (choice) {
                case 1:
                    showPhone();
                    break;
                case 2:
                    addPhone();
                    break;
                case 3:
                    updatePhone();
                    break;
                case 4:
                    deletePhone();
                    break;
                case 5:
                    sortPhone();
                    break;
                case 6:
                    searchPhone();
                    break;
                case 7:
                    break;
                case 8:
                    break;
                case 9:
                    System.out.println("Đã thoát chương trình.");
                    System.exit(0);
                    break;
                default:
                    System.out.println("Chức năng không tồn tại.");
                    guiMain();
            }
        } while (choice != 9);
    }

    public void showPhone() {
        do {
            System.out.println("-- Xem danh sách điện thoại --");
            System.out.println("1. Tất cả.");
            System.out.println("2. Điện thoại cũ.");
            System.out.println("3. Điện thoại mới.");
            System.out.println("4. Quay lại.");
            System.out.print("Chọn chức năng: ");
            choice = Integer.parseInt(sc.next());
            String choiceStr;
            switch (choice) {
                case 1:
                    System.out.println("-- Danh sách điện thoại --");
                    outputAllPhone();
                    System.out.print("Nhập bất kỳ để tiếp tục: ");
                    choiceStr = sc.next();
                    if (!choiceStr.isEmpty()) {
                        break;
                    }
                case 2:
                    System.out.println("-- Danh sách điện thoại cũ --");
                    for (SecondHandPhone secondHandPhone : secondHandPhones) {
                        secondHandPhone.outputPhone();
                    }
                    System.out.print("Nhập bất kỳ để tiếp tục: ");
                    choiceStr = sc.next();
                    if (!choiceStr.isEmpty()) {
                        break;
                    }
                case 3:
                    System.out.println("-- Danh sách điện thoại mới --");
                    System.out.println("ID\t|Tên\t|Hãng\t|Giá\t|Bảo hành");
                    for (NewPhone newPhone : newPhones) {
                        newPhone.outputPhone();
                    }
                    System.out.print("Nhập bất kỳ để tiếp tục: ");
                    choiceStr = sc.next();
                    if (!choiceStr.isEmpty()) {
                        break;
                    }
                case 4:
                    guiMain();
                    break;
                default:
                    System.out.println("Chức năng không tồn tại.");
            }
        } while (choice != 4);
    }

    public void addPhone() {
        do {
            System.out.println("-- Thêm điện thoại --");
            System.out.println("1. Điện thoại cũ.");
            System.out.println("2. Điện thoại mới.");
            System.out.println("3. Quay lại.");
            System.out.print("Chọn chức năng: ");
            choice = Integer.parseInt(sc.next());
            switch (choice) {
                case 1:
                    secondHandPhones.add(new SecondHandPhone());
                    secondHandPhones.get(n).inputPhone();
                    n++;
                    break;
                case 2:
                    newPhones.add(new NewPhone());
                    newPhones.get(m).inputPhone();
                    m++;
                    break;
                case 3:
                    guiMain();
                    break;
                default:
                    System.out.println("Chức năng không tồn tại.");
            }
        } while (choice != 3);
    }

    public void sortPhone() {
        do {
            System.out.println("-- Sắp xếp theo giá --");
            System.out.println("1. Sắp xếp tăng dần.");
            System.out.println("2. Sắp xếp giảm dần.");
            System.out.println("3. Quay lại.");
            System.out.print("Chọn chức năng: ");
            choice = Integer.parseInt(sc.next());
            String choiceStr;
            switch (choice) {
                case 1:
                    Collections.sort(newPhones);
                    Collections.sort(secondHandPhones);
                    System.out.println("Sắp xếp tăng dần thành công.");
                    outputAllPhone();
                    System.out.print("Nhập bất kỳ để tiếp tục: ");
                    choiceStr = sc.next();
                    if (!choiceStr.isEmpty()) {
                        break;
                    }
                case 2:
                    newPhones.sort(Collections.reverseOrder());
                    secondHandPhones.sort(Collections.reverseOrder());
                    System.out.println("Sắp xếp giảm dần thành công.");
                    outputAllPhone();
                    System.out.print("Nhập bất kỳ để tiếp tục: ");
                    choiceStr = sc.next();
                    if (!choiceStr.isEmpty()) {
                        break;
                    }
                case 3:
                    guiMain();
                    break;
                default:
                    System.out.println("Chức năng không tồn tại.");
            }
        } while (choice != 3);
    }

    public void searchPhone() {
        do {
            System.out.println("-- Tìm kiếm điện thoại --");
            System.out.println("1. Tìm kiếm tất cả các loại điện thoại.");
            System.out.println("2. Tìm kiếm điện thoại cũ.");
            System.out.println("3. Tìm kiếm điện thoại mới.");
            System.out.println("4. Quay lại..");
            System.out.print("Chọn chức năng: ");
            choice = Integer.parseInt(sc.next());
            switch (choice) {
                case 1:
                    findPhone();
                    break;
                case 2:
                    findSecondHandPhone();
                    break;
                case 3:
                    findNewPhone();
                    break;
                case 4:
                    guiMain();
                    break;
                default:
                    System.out.println("Chức năng không tồn tại.");
            }
        } while (choice != 4);
    }

    public void findPhone() {
        do {
            System.out.println("-- Tìm kiếm điện thoại --");
            System.out.println("1. Tìm kiếm theo giá.");
            System.out.println("2. Tìm kiếm theo tên.");
            System.out.println("3. Tìm kiếm theo hãng.");
            System.out.println("4. Quay lại.");
            System.out.print("Chọn chức năng: ");
            choice = Integer.parseInt(sc.next());
            String choiceStr;
            switch (choice) {
                case 1:
                    BigDecimal minPrice, maxPrice;
                    System.out.print("Giá từ: ");
                    minPrice = new BigDecimal(sc.next());
                    System.out.print("đến: ");
                    maxPrice = new BigDecimal(sc.next());
                    System.out.println("-- Kết quả tìm kiếm --");
                    System.out.println("Danh sách điện thoại mới: ");
                    for (NewPhone newPhone : newPhones) {
                        if (newPhone.getPrice().compareTo(minPrice) >= 0 && newPhone.getPrice().compareTo(maxPrice) <= 0) {
                            newPhone.outputPhone();
                        }
                    }
                    System.out.println("Danh sách điện thoại cũ: ");
                    for (SecondHandPhone secondHandPhone : secondHandPhones) {
                        if (secondHandPhone.getPrice().compareTo(minPrice) >= 0 && secondHandPhone.getPrice().compareTo(maxPrice) <= 0) {
                            secondHandPhone.outputPhone();
                        }
                    }
                    System.out.print("Nhập bất kỳ để tiếp tục: ");
                    choiceStr = sc.next();
                    if (!choiceStr.isEmpty()) {
                        break;
                    }
                case 2:
                    String name;
                    System.out.print("Nhập tên điện thoại: ");
                    name = sc.next();
                    System.out.println("-- Kết quả tìm kiếm --");
                    System.out.println("Danh sách điện thoại mới: ");
                    for (NewPhone newPhone : newPhones) {
                        if (newPhone.getModel().contains(name)) {
                            newPhone.outputPhone();
                        }
                    }
                    System.out.println("Danh sách điện thoại cũ: ");
                    for (SecondHandPhone secondHandPhone : secondHandPhones) {
                        if (secondHandPhone.getModel().contains(name)) {
                            secondHandPhone.outputPhone();
                        }
                    }
                    System.out.print("Nhập bất kỳ để tiếp tục: ");
                    choiceStr = sc.next();
                    if (!choiceStr.isEmpty()) {
                        break;
                    }
                case 3:
                    String brand;
                    System.out.print("Nhập hãng điện thoại: ");
                    brand = sc.next();
                    System.out.println("-- Kết quả tìm kiếm --");
                    System.out.println("Danh sách điện thoại mới: ");
                    for (NewPhone newPhone : newPhones) {
                        if (newPhone.getBrand().contains(brand)) {
                            newPhone.outputPhone();
                        }
                    }
                    System.out.println("Danh sách điện thoại cũ: ");
                    for (SecondHandPhone secondHandPhone : secondHandPhones) {
                        if (secondHandPhone.getBrand().contains(brand)) {
                            secondHandPhone.outputPhone();
                        }
                    }
                    System.out.print("Nhập bất kỳ để tiếp tục: ");
                    choiceStr = sc.next();
                    if (!choiceStr.isEmpty()) {
                        break;
                    }
                case 4:
                    searchPhone();
                    break;
                default:
                    System.out.println("Chức năng không tồn tại.");
            }
        } while (choice != 4);
    }

    public void findSecondHandPhone() {
        do {
            System.out.println("-- Tìm kiếm điện thoại cũ --");
            System.out.println("1. Tìm kiếm theo giá.");
            System.out.println("2. Tìm kiếm theo tên.");
            System.out.println("3. Tìm kiếm theo hãng.");
            System.out.println("4. Quay lại.");
            System.out.print("Chọn chức năng: ");
            choice = Integer.parseInt(sc.next());
            String choiceStr;
            switch (choice) {
                case 1:
                    BigDecimal minPrice, maxPrice;
                    System.out.print("Giá từ: ");
                    minPrice = new BigDecimal(sc.next());
                    System.out.print("đến: ");
                    maxPrice = new BigDecimal(sc.next());
                    System.out.println("-- Kết quả tìm kiếm --");
                    System.out.println("Danh sách điện thoại cũ: ");
                    for (SecondHandPhone secondHandPhone : secondHandPhones) {
                        if (secondHandPhone.getPrice().compareTo(minPrice) >= 0 && secondHandPhone.getPrice().compareTo(maxPrice) <= 0) {
                            secondHandPhone.outputPhone();
                        }
                    }
                    System.out.print("Nhập bất kỳ để tiếp tục: ");
                    choiceStr = sc.next();
                    if (!choiceStr.isEmpty()) {
                        break;
                    }
                case 2:
                    String name;
                    System.out.print("Nhập tên điện thoại: ");
                    name = sc.next();
                    System.out.println("-- Kết quả tìm kiếm --");
                    System.out.println("Danh sách điện thoại cũ: ");
                    for (SecondHandPhone secondHandPhone : secondHandPhones) {
                        if (secondHandPhone.getModel().contains(name)) {
                            secondHandPhone.outputPhone();
                        }
                    }
                    System.out.print("Nhập bất kỳ để tiếp tục: ");
                    choiceStr = sc.next();
                    if (!choiceStr.isEmpty()) {
                        break;
                    }
                case 3:
                    String brand;
                    System.out.print("Nhập hãng điện thoại: ");
                    brand = sc.next();
                    System.out.println("-- Kết quả tìm kiếm --");
                    System.out.println("Danh sách điện thoại cũ: ");
                    for (SecondHandPhone secondHandPhone : secondHandPhones) {
                        if (secondHandPhone.getBrand().contains(brand)) {
                            secondHandPhone.outputPhone();
                        }
                    }
                    System.out.print("Nhập bất kỳ để tiếp tục: ");
                    choiceStr = sc.next();
                    if (!choiceStr.isEmpty()) {
                        break;
                    }
                case 4:
                    searchPhone();
                    break;
                default:
                    System.out.println("Chức năng không tồn tại.");
            }
        } while (choice != 4);
    }

    public void findNewPhone() {
        do {
            System.out.println("-- Tìm kiếm điện thoại mới --");
            System.out.println("1. Tìm kiếm theo giá.");
            System.out.println("2. Tìm kiếm theo tên.");
            System.out.println("3. Tìm kiếm theo hãng.");
            System.out.println("4. Quay lại.");
            System.out.print("Chọn chức năng: ");
            choice = Integer.parseInt(sc.next());
            String choiceStr;
            switch (choice) {
                case 1:
                    BigDecimal minPrice, maxPrice;
                    System.out.print("Giá từ: ");
                    minPrice = new BigDecimal(sc.next());
                    System.out.print("đến: ");
                    maxPrice = new BigDecimal(sc.next());
                    System.out.println("-- Kết quả tìm kiếm --");
                    System.out.println("Danh sách điện thoại mới: ");
                    for (NewPhone newPhone : newPhones) {
                        if (newPhone.getPrice().compareTo(minPrice) >= 0 && newPhone.getPrice().compareTo(maxPrice) <= 0) {
                            newPhone.outputPhone();
                        }
                    }
                    System.out.print("Nhập bất kỳ để tiếp tục: ");
                    choiceStr = sc.next();
                    if (!choiceStr.isEmpty()) {
                        break;
                    }
                case 2:
                    String name;
                    System.out.print("Nhập tên điện thoại: ");
                    name = sc.next();
                    System.out.println("-- Kết quả tìm kiếm --");
                    System.out.println("Danh sách điện thoại mới: ");
                    for (NewPhone newPhone : newPhones) {
                        if (newPhone.getModel().contains(name)) {
                            newPhone.outputPhone();
                        }
                    }
                    System.out.print("Nhập bất kỳ để tiếp tục: ");
                    choiceStr = sc.next();
                    if (!choiceStr.isEmpty()) {
                        break;
                    }
                case 3:
                    String brand;
                    System.out.print("Nhập hãng điện thoại: ");
                    brand = sc.next();
                    System.out.println("-- Kết quả tìm kiếm --");
                    System.out.println("Danh sách điện thoại mới: ");
                    for (NewPhone newPhone : newPhones) {
                        if (newPhone.getBrand().contains(brand)) {
                            newPhone.outputPhone();
                        }
                    }
                    System.out.print("Nhập bất kỳ để tiếp tục: ");
                    choiceStr = sc.next();
                    if (!choiceStr.isEmpty()) {
                        break;
                    }
                case 4:
                    searchPhone();
                    break;
                default:
                    System.out.println("Chức năng không tồn tại.");
            }
        } while (choice != 4);
    }
}
